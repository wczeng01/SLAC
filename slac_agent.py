...
from ...utils import common as slac_common
from ...utils import gif_utils
from ...utils import nest_utils as slac_nest_utils
  ...
  for t, (time_step, action) in enumerate(zip(time_steps, actions)):
    if t == 0:
      ...
      if action is not None:
                reset_mask_broadcast = tf.broadcast_to(reset_mask_broadcast, action.shape)
                action = tf.where(reset_mask_broadcast, tf.zeros_like(action), action)
