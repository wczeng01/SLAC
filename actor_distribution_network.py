...
class SquashToSpecDistribution(tfp.distributions.TransformedDistribution):
  def __init__(self, distribution, spec, name="SquashToSpecDistribution"):
    self.action_means, self.action_magnitudes = common.spec_means_and_magnitudes(
        spec)
    shift_bijector = tfp.bijectors.Shift(shift=self.action_means)
    scale_bijector = tfp.bijectors.Scale(scale=self.action_magnitudes)
    affine_bijector = tfp.bijectors.Chain([scale_bijector, shift_bijector])
    bijectors = [affine_bijector, tanh_bijector_stable.Tanh()]
