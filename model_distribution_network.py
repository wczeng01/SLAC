  ...
  def sample_posterior(self, images, actions, step_types, features=None):
    ...
    for t in range(sequence_length + 1):
      if t == 0:
        ...
        latent1_dist = self.latent1_posterior(features[t], latent2_samples[t-1], actions[t-1])
        # new lines below:
        reset_mask_broadcast = tf.reshape(reset_mask, [-1, 1])
        reset_mask_broadcast = tf.broadcast_to(reset_mask_broadcast, latent1_first_dist.mean().shape)
        # new lines above
        latent1_dist = nest_utils.map_distribution_structure(
            functools.partial(tf.where, reset_mask_broadcast), latent1_first_dist, latent1_dist)
        latent1_sample = latent1_dist.sample()
        ...
        # new lines below:
        reset_mask_broadcast = tf.reshape(reset_mask, [-1, 1])
        reset_mask_broadcast = tf.broadcast_to(reset_mask_broadcast, latent2_first_dist.mean().shape)
        # new lines above
        latent2_dist = nest_utils.map_distribution_structure(
            functools.partial(tf.where, reset_mask_broadcast), latent2_first_dist, latent2_dist)
        latent2_sample = latent2_dist.sample()
